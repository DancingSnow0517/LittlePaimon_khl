...
async def check_token(uuid: str, cookie: str):
    ...
    req = await requests.post(host + 'hk4e_cg_cn/gamer/api/login', headers=headers)
    data = json.loads(req)
...
async def get_Info(uuid: str, cookie: str):
    headers = get_header(uuid, cookie)
    req = await requests.get(host + 'hk4e_cg_cn/wallet/wallet/get', headers=headers)
    return json.loads(req)


async def get_Announcement(uuid: str, cookie: str):
    headers = get_header(uuid, cookie)
    req = await requests.get(host + 'hk4e_cg_cn/gamer/api/getAnnouncementInfo', headers=headers)
    return json.loads(req)
