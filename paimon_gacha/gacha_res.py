...
def once(uid, gacha_data):
    ...
    if rank == 3:
        ...
        if rank == 5 and pool_str == 'weapon' and user_info[uid]["gacha_list"][
            "dg_time"] == 2 and "pool_type" not in gacha_data:
        ...
        if gacha_data['gacha_type'] != 200:
            user_info[uid]["gacha_list"][("is_up_%s_%s" % (rank, pool_str))] = not up
...
async def more_ten(uid, gacha_data, num, user: User):
    ...
    for _ in range(num):
        ...
        draw.text((27, 575 - 30), ('@%s %s  Created By LittlePaimon' % (str(user.nickname), time_str)),
                  font=time_font, fill="#8E8E8E")
        img_list.append(img)
    return img_list
