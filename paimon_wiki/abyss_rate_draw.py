...
from paimon_wiki.abyss_rate_data import get_rate, get_formation_rate
...
async def draw_rate_rank(type: str = 'role', mode: str = 'used'):
    ...
    col = int(len(data['result']['rateList']) / 5) if len(data['result']['rateList']) % 5 == 0 else int(
        len(data['result']['rateList']) / 5) + 1
